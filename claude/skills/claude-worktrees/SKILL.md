...
スクリプトはClaudeセッションがあるworktreeだけを返す。ただし、ユーザーが **現在メインワークツリーにいる場合はメインworktreeのエントリは返さない**（いま自分がいる場所の情報は冗長なので）。以下のフォーマットで、worktreeごとにまとめて表示する。見出し（`###`）で区切り、情報が密集しないように整形する。
