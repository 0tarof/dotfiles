...
- `hosts/darwin/default.nix`: Homebrew formula exceptions and GUI casks managed by nix-darwin.
...
2. GUI macOS apps: use `hosts/darwin/default.nix` Homebrew casks.
...
4. Homebrew formulae: use `hosts/darwin/default.nix` only when Nix is missing, broken, macOS-specific, or materially worse for this repo.
5. External flakes: use `flake.nix` when upstream provides a flake and this repo needs that exact source or package output.
6. Manual curl/download: use only as a last resort, and prefer wrapping it in a declarative activation script if it must be used.
...
- Recommend a Homebrew cask when the app is a GUI macOS application.
