...
{
  # ==========================================================================
  # Docker Desktop の残骸掃除
  # ==========================================================================
  # docker CLI は nixpkgs (docker-client) 管理に移行したので、Docker.app を指す
  # 壊れた cli-plugins symlink と Desktop 生成の completion は不要。
  # 前者は dotfiles.nix の cli-plugins リンクと衝突するため、
  # home.file の衝突チェック (checkLinkTargets) より前に消す。
  home.activation.pruneDockerDesktopLeftovers = lib.hm.dag.entryBefore [ "checkLinkTargets" ] ''
    if [[ -z "''${DRY_RUN:-}" ]]; then
      for link in "$HOME/.docker/cli-plugins"/*; do
        if [[ -L "$link" && ! -e "$link" ]]; then
          rm -f "$link"
        fi
      done
      rm -f "$HOME/.docker/completions/_docker"
    fi
  '';
  ...
  programs.zsh = {
    ...
    profileExtra = ''
      ...
      # Homebrew に GitHub トークンを渡さない。渡すと tap の git 操作が
      # https://x-access-token:TOKEN@github.com/... に書き換えられ、
      # url.insteadOf の SSH 書き換えを外れて osxkeychain にトークンが残る。
      # 非公式 tap の clone/fetch は insteadOf 経由の SSH 認証で通るため不要。
